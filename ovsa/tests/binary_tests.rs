...
fn test_sparse_random() {
    let dimension = 10;
    let n_active = 3;
    let vec = ovsa::binary::sparse_random(dimension, n_active);
...
fn test_cyclic_shift() {
    let dimension = 10;
    let vec = ovsa::binary::from_indices(dimension, &[1, 3, 5, 9]);
...
fn test_cyclic_shift_negative() {
    let dimension = 10;
    let vec = ovsa::binary::from_indices(dimension, &[1, 3, 5, 0]);
    let shifted_vec = ovsa::binary::cyclic_shift(&vec, -2);
...
fn test_similarity() {
    ...
    let vec1 = ovsa::binary::from_indices(dimension, &[1, 3, 5]);
    let vec2 = ovsa::binary::from_indices(dimension, &[3, 4, 5]);
    let similarity = ovsa::binary::similarity(&vec1, &vec2);
